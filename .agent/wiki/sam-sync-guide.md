...
- **Sam Public** (`~$HOME/Projects/sam/`) - Public template that must stay sanitized
...
cp -r ${PAI_DIR}/skills/new-skill ~$HOME/Projects/sam/.claude/skills/

# Example: Updating a hook
cp ${PAI_DIR}/hooks/some-hook.ts ~$HOME/Projects/sam/.claude/hooks/
```

**IMPORTANT:** Do NOT use `cp -r ~/.claude ~$HOME/Projects/sam/` (don't bulk copy everything)
...
```bash
# ❌ DON'T DO THIS
cp -r ${PAI_DIR}/* ~$HOME/Projects/sam/.claude/
```
...
cp ${PAI_DIR}/../README.md ~$HOME/Projects/sam/README.md
```
...
```bash
# 1. Test Sam works
bun ~$HOME/Projects/sam/.claude/hooks/self-test.ts

# 2. Validate protected files
bun ~$HOME/Projects/sam/.claude/hooks/validate-protected.ts
...
cp -r ${PAI_DIR}/skills/my-new-skill ~$HOME/Projects/PAI/.claude/skills/

# 2. Sanitize the skill's SKILL.md
cd ~$HOME/Projects/PAI/.claude/skills/my-new-skill
