...
- **Issue:** 19 project memory directories from previous users/machines
  - `/*` - Old macOS paths
  - `/*` - Previous username directories (now archived)
  - Only `/*` is current
...
- **Recommendation:** Create `/.claude/SKILLS_INDEX.md` with categorized listing
...
- **Recommendation:** Create `/.claude/health-check.sh` that verifies all services
...
sudo cp /.claude/skills/*/pai-*.service /etc/systemd/system/
...
```bash
# Create /.claude/projects/_archive/
# Move all  and  (legacy) dirs
...
|------|---|---|
| `/.claude/CLAUDE.md` | ✅ Good | Add Linux setup notes |
| `/.claude/settings.json` | ⚠️ Needs review | Consider splitting by env |
| `/.claude/.env` | ✅ OK | Add Discord token validation |
| `/.claude/startup` | ⚠️ Incomplete | Add systemd support |
