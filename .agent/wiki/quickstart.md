...
```bash
git clone https://github.com/danielmiessler/Sam.git ~/sam
```
...
- Pull updates with `cd ~/sam && git pull`
...
- **Bun install** — Offers to install Bun if not found (Sam's package manager)
...
```powershell
git clone https://github.com/danielmiessler/Sam.git $env:USERPROFILE\sam
```
...
New-Item -ItemType SymbolicLink -Path "$env:USERPROFILE\.claude" -Target "$env:USERPROFILE\sam\.claude"
```
...
Sam loads automatically via the `SessionStart` hook.
...
<details>
<summary><strong>Sam Not Loading</strong></summary>
