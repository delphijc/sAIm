...
async function main() {
  ...
  // Send notification with intelligent fallback (voice → system)
  const result = await sendNotification({
    title: `${agentName} Agent`,
    message: fullMessage,
    voiceId: AGENT_VOICE_IDS[finalAgentType] || AGENT_VOICE_IDS.default,
    agentType: finalAgentType
  });

  console.log(`✅ Sent via ${result.method}: [${agentName}] ${fullMessage}`);
}
