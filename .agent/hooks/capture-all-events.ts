...
import { sanitizePayload } from './lib/sanitize-credentials';
...
function getAgentForSession(sessionId: string): string {
  try {
    ...
    if (existsSync(mappingFile)) {
      ...
      return mappings[sessionId] || 'Sam';
    }
  } catch (error) {
    // Ignore errors, default to Sam
  }
  return 'Sam';
}
...
async function main() {
  try {
    ...
    // If this is a SubagentStop or Stop event, reset to Sam
    else if (eventType === 'SubagentStop' || eventType === 'Stop') {
      agentName = 'Sam';
      setAgentForSession(sessionId, 'Sam');
    }
    ...
    // Sanitize credentials from payload before persisting
    const sanitizedEvent = sanitizePayload(event);

    // Append to events file
    const eventsFile = getEventsFilePath();
    const jsonLine = JSON.stringify(sanitizedEvent) + '\n';
