 ...
 *
 * Ensures PAI-specific files haven't been overwritten with Sam content.
...
function checkFileContent(filePath: string, manifest: ProtectedManifest): {
  ...
  for (const [categoryName, category] of Object.entries(manifest.protected)) {
    ...
    // Must not contain private Sam data (skip if exception file)
    if (category.validation.includes('private Sam data') && !isException) {
      const privatePatterns = [
        /\/Users\/daniel\/\.claude\/skills\/personal/,
        /daemon\.plist/,
        /Sam \(Personal AI Infrastructure\)/,
      ];

      for (const pattern of privatePatterns) {
        if (pattern.test(content)) {
          violations.push(`Contains private Sam reference: ${pattern.source}`);
        }
      }
    }
...
async function main() {
  ...
  if (hasViolations) {
    ...
    console.log('Safe to commit to Sam repository.\n');
    process.exit(0);
  }
}
