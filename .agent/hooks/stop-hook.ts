...
import { sendNotification, isVoiceServerRunning } from './lib/notification-handler';
...
// Load voices configuration (local ~/.claude location)
...
try {
  ...
  VOICE_CONFIG = {
    default_rate: 175,
    voices: {
      main: { voice_name: "Jamie (Premium)", rate_wpm: 228, rate_multiplier: 1.3, description: "UK Male", type: "Premium" },
      researcher: { voice_name: "Ava (Premium)", rate_wpm: 236, rate_multiplier: 1.35, description: "US Female", type: "Premium" },
      engineer: { voice_name: "Zoe (Premium)", rate_wpm: 236, rate_multiplier: 1.35, description: "US Female", type: "Premium" },
      ...
      writer: { voice_name: "Serena (Premium)", rate_wpm: 236, rate_multiplier: 1.35, description: "UK Female", type: "Premium" }
    }
  };
}
...
async function main() {
  ...
  let lastAssistantMessageDirect: string | undefined;
  try {
    const parsed = JSON.parse(input);
    transcriptPath = parsed.transcript_path;
    // Use last_assistant_message field if available (new in Claude Code Feb/Mar 2026)
    // This avoids reading and parsing the full transcript file
    if (parsed.last_assistant_message) {
      lastAssistantMessageDirect = typeof parsed.last_assistant_message === 'string'
        ? parsed.last_assistant_message
        : contentToText(parsed.last_assistant_message);
    }
  } catch (e) {
    process.exit(0);
  }
  ...
  let samHasCustomCompleted = false;

  // ALWAYS check Sam's response FIRST (even when agents are used)
  // Prefer last_assistant_message from hook input if available (avoids transcript re-parsing)
  const lastResponse = lines[lines.length - 1];
  const lastAssistantContent = lastAssistantMessageDirect ?? (() => {
    try {
  ...
  if (message) {
    ...
    // Cap voice message at ~100 words, cutting at a sentence boundary
    const words = message.split(/\s+/);
    let voiceMessage = message;
    if (words.length > 100) {
      // Find the last sentence-ending punctuation within the first 100 words
      const truncated = words.slice(0, 100).join(' ');
      const sentenceEnd = truncated.search(/[.!?][^.!?]*$/);
      voiceMessage = sentenceEnd !== -1
        ? truncated.slice(0, sentenceEnd + 1)
        : truncated;
      console.error(`✂️ Voice message trimmed at sentence boundary (${words.length} → ${voiceMessage.split(/\s+/).length} words)`);
    }
  ...
  if (!tabTitle && lastUserQuery) {
    ...
    // Re-use the already-resolved lastAssistantContent (avoids re-parsing)
    try {
      if (lastAssistantContent) {
        const completedMatch = lastAssistantContent.match(/🎯\s*COMPLETED:\s*(.+?)(?:\n|$)/im);
        if (completedMatch) {
