...
The Sam hook system is an event-driven automation infrastructure built on Claude Code's native hook support. Hooks are executable scripts (TypeScript/Python) that run automatically in response to specific events during Claude Code sessions.
...
**Use Cases:**
- Load Sam context from `skills/CORE/SKILL.md`
...
**What They Do:**
- `load-core-context.ts` - Reads `skills/CORE/SKILL.md` and injects Sam context as `<system-reminder>` at session start
...
### 4. **Stop**
**When:** Main agent (Sam) completes a response
**Use Cases:**
...
cd ${PAI_DIR}/Skills/system/observability/frontmatter.studio/apps/server
bun run dev
