# SAM SYSTEM CONSTITUTION

**The Foundational Philosophy, Architecture, and Operations of Sam**

**Last Updated:** 2025-11-17
**Status:** Active - This is the canonical reference for all Sam architectural decisions
...
**Sam is scaffolding for AI, not a replacement for human intelligence.**

The system is designed on the principle that **AI systems need structure to be reliable**. Like physical scaffolding supports construction work, Sam provides the architectural framework that makes AI assistance dependable, maintainable, and effective.
...
A well-structured system with good scaffolding will outperform a more powerful model with poor structure. Sam's value comes from:
...
**Key Takeaway:** Skills are how Sam scales - each new domain gets its own skill, maintaining organization as the system grows.
...
name: CORE
description: |
  Sam core identity and infrastructure. Loaded at session start.
...
**THE MOST IMPORTANT ARCHITECTURAL PATTERN IN SAM**
...
**The building blocks of Sam's architecture.**
...
Frontmatter:
- voice_id: [ChatterboxTTS voice ID, e.g. jessica]
...
- Profile-based configuration in Sam
...
**See [Two-Tier MCP Strategy](#two-tier-mcp-strategy) for Sam's approach to MCPs.**
...
Sam uses mandatory structured output format that integrates with voice server for spoken feedback.
...
- **Voice Integration:** This line is spoken aloud via ChatterboxTTS (voice-server)
- **User Feedback:** User hears completion via agent-specific voice
- **Event Logging:** Captured to history/raw-outputs/
- **Status Tracking:** Enables observability dashboard

**Voice Integration Flow:**

1. **Sam/Agent completes task**
   ```markdown
...
2. **Stop hook fires** (`${PAI_DIR}/Hooks/stop-hook.ts`)
   ...
   - Determines entity (Sam vs specific agent)
...
4. **Voice server processes** (`${PAI_DIR}/voice-server/server.ts`)
   ...
   - Routes to ChatterboxTTS Python sidecar with voice_id
   - Receives WAV audio
...
```
Sam → Engineer → Implementation complete
```

**Parallel Delegation:**
```
Sam → [Intern1, Intern2, Intern3] → All complete → Sam synthesizes
```

**Nested Delegation:**
```
Sam → Architect (designs) → Engineer (implements) → Sam verifies
```

**Spotcheck Pattern:**
```
Sam → [10 Interns update files] → Spotcheck Intern (verifies all 10)
```
...
| security | minimal + playwright | Security testing |
...
│   ├── CORE/                        # Sam identity + infrastructure
│   │   ├── SKILL.md                 # Main Sam skill (Tier 2)
...
│
├── voice-server/                    # ChatterboxTTS integration (via voice-server)
...
## Voice Configuration
**Voice ID:** [ChatterboxTTS voice ID, e.g. jessica]
...
### The Ten Commandments of Sam Architecture
...
**These are non-negotiable foundations that ensure Sam remains dependable, maintainable, and effective.**
...
**This document defines what Sam is and how Sam works at the most fundamental level.**
