...
function getTodayEventsFile(): string {
  ...
  if (process.env.PAI_DIR) {
    // Hooks write to $PAI_DIR/history/raw-outputs/ - use this directly
    historyDir = join(process.env.PAI_DIR, 'history', 'raw-outputs');
  } else if (process.env.SAM_DIR) {
    // Fallback to SAM_DIR/.agent/history/raw-outputs
    historyDir = join(process.env.SAM_DIR, '.agent', 'history', 'raw-outputs');
  } else {
    // Default: assume sam project is at ~/Projects/sam
    historyDir = join(homedir(), 'Projects', 'sam', '.agent', 'history', 'raw-outputs');
  }
...
function loadAgentSessions(): void {
  ...
  if (!existsSync(sessionsFile)) {
    console.log(`⚠️  agent-sessions.json not found at ${sessionsFile}, agent names will be "unknown"`);
    return;
  }
...
function enrichEventWithAgentName(event: HookEvent): HookEvent {
  ...
  // Default to 'sam' for main agent sessions (not 'unknown')
  const agentName = agentSessions.get(event.session_id) || 'sam';
  return {
    ...event,
...
export function startFileIngestion(callback?: (events: HookEvent[]) => void): void {
  ...
  console.log(`📂 Reading from ${historyDir}`);
