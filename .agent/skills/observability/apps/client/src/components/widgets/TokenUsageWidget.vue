      ...
      <div v-else class="stats-container">
        ...
        <div v-if="hasAgentData" class="section">
          ...
          <div class="agent-list">
            <div
              v-for="([agent, data]) in sortedAgents"
              :key="agent"
              class="agent-item"
            >
...
function getServiceColor(_service: string, index: number): string {
...
async function fetchStats() {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/api/token-usage/stats${timeRangeParam}`);
