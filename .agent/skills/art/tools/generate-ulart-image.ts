...
OPTIONS:
  ...
  --creative-variations <n>  Generate N variations (appends -v1, -v2, etc. to output filename)
                             Use with Sam's be-creative skill for true prompt diversity
...
NOTE: For true creative diversity with different prompts, use the creative workflow in Sam which
...
async function generateWithNanoBananaPro(
  ...
  if (referenceImage) {
    console.log(`🍌✨ Generating with Nano Banana Pro (Gemini 2.5 Flash Image) at ${size} ${aspectRatio} with reference image...`);
  } else {
    console.log(`🍌✨ Generating with Nano Banana Pro (Gemini 2.5 Flash Image) at ${size} ${aspectRatio}...`);
  }
...
async function main(): Promise<void> {
  try {
    ...
    if (args.creativeVariations && args.creativeVariations > 1) {
      ...
      console.log(`   For true creative diversity, use the creative workflow in Sam with be-creative skill\n`);
